...
export const getStatsForIdentity = (
  ...
  getStatsBySource(identity.apiKey ?? '', keyStats, identity.prefix, identity.authIndex);
...
export const collectUsageDetailsForIdentity = (
  ...
  if (authIndexKey) {
    const details = usageDetails.filter(
      (detail) => normalizeAuthIndex(detail.auth_index) === authIndexKey
    );
    if (details.length > 0) {
      return details;
    }
  ...
  return usageDetails.filter((detail) => candidateSet.has(detail.source));
};
...
// 对于 OpenAI 提供商，汇总所有 apiKeyEntries 的统计
...
export const collectOpenAIProviderUsageDetails = (
  ...
  if (!normalizeAuthIndex(provider.authIndex) && provider.prefix) {
    groups.push(collectUsageDetailsForIdentity({ prefix: provider.prefix }, usageDetails));
  }
